...
crossValidation = list(outer = list(method = "Holdout",  predict = "both"), 
                                 inner = list(method = "Holdout"))

run = TRUE
```


### Network Size with species abundance
Create Simulation:
```{r,results=FALSE}
set.seed(42)
simulatedData = simulateInteraction( main = NULL, inter = matrix(c("A1", "B1",
                              ...
                              weights = list(inter = c(200,200,200)),
                              NumberA = 100, NumberB = 200, traitsA = c(0,6), 
                              traitsB = c(0,6),abundance = function(a,b) return(rexp(a, 2)), specRange = c(0.5,1.2))
...
```{r,results=FALSE}
interBaseP = minOneInter(simulatedData$poisson(2e-4))
interBaseC = minOneInter(simulatedData$binar(2e-4))
...
if(run){
  ...
  for(i in 1:length(sizeC)){
    resultC[[i]] = runTM(createCommunity(simulatedData$A, simulatedData$B, sizeC[[i]]), 
                         method = c("dnn", "cnn", "knn", "SVM", "naive", "RFranger", "boost", "glm"), 
  ...
  for(i in 1:length(sizeC)){
    resultR[[i]] = runTM(createCommunity(simulatedData$A, simulatedData$B, sizeR[[i]], log = F), 
                         method = c("negBinDnn", "RFranger", "SVM", "knn", "boost", "glm"), 
          settings = list(negBin = list(seed = 42, nLayer = 3L, batch = 50, 
...
### Observation time
Varying observation times results in diffrent class distributions (classification). I choose times so, that the positive interaction had class proportions of 10 \%, 20-25 \% and 50 \%:
...
obsC = list(low = minOneInter(simulatedData$binar(1e-6)), 
            mid = minOneInter(simulatedData$binar(3e-6)), 
            high = minOneInter(simulatedData$binar(1.3e-5)))

obsR = list(low = minOneInter(simulatedData$poisson(1e-6)), 
            mid = minOneInter(simulatedData$poisson(3e-6)), 
            high = minOneInter(simulatedData$poisson(1.3e-5)))
```
...
### Abundance
We assumed that the observer had no information about abundances. Here, We set abundances to uniform to test whether the performances increases. Algorithms were tested again on the three different network sizes:
```{r,results=FALSE}
set.seed(42)
simulatedData = simulateInteraction( main = NULL, inter = matrix(c("A1", "B1",
                                                                   "A2", "B2",
                              ...
                              NumberA = 100, NumberB = 200, traitsA = c(0,6), 
                              traitsB = c(0,6),abundance = FALSE, specRange = c(0.5,1.2))
interBaseP = minOneInter(simulatedData$poisson(1.3e-5))
interBaseC = minOneInter(simulatedData$binar(1.3e-5))
smallP = interBaseP[sample(rownames(interBaseP), 0.25*length(rownames(interBaseP))),
                    sample(colnames(interBaseP), 0.25*length(colnames(interBaseP)))]
smallC = interBaseC[rownames(smallP), colnames(smallP)]
...
if(run){
  for(i in 1:length(sizeC)){
    resultC[[i]] = runTM(createCommunity(simulatedData$A, simulatedData$B, sizeC[[i]]), 
          ...
          settings = list(dnn = list(seed = 42,  activation = "relu", drop = 0.2, archFunction = "continous", epochs = epochs),
                       ...
                       crossValidation = crossValidation, 
                       balanceClasses = "None", block = T, parallel = parallel, seed = 42, keepModels = T)
    saveRDS(resultC, file = "../Results/simAbC.RDS")
  }
  ...
  for(i in 1:length(sizeC)){
    ...
    saveRDS(resultR, file = "../Results/simAbR.RDS")
  }
}
```
