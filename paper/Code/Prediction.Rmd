...
if(run){
  ...
  result = runTM(cC, tune = "random", iters = iter, tuningMetric = "auc", crossValidation = cv, block = T, parallel = parallel, fitSpecies = F, balanceClasses = c("Over", "SMOTE", "Under"),
                 ...
                 seed = 42,settings =  list(
                   dnn = list(seed = 42, opti = "sgd", drop = 0.3, archFunction = "continous",batch = 50L, epochs = epochs),
                   cnn = list(seed = 42, opti = "sgd", drop = 0.3, archFunction = "continous",batch = 50L, kernel_size = 2, epochs = epochs)
                 ))
