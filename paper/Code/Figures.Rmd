...
text(x =  c(-8,0,9), y = rep(-0.22,3), pos = 4, 
    labels = rev(c("TraitMatching, even species abundances ", 
...
pdf("Figures/FigS2_new.pdf",width = 5.5, height = 4.6, compress = FALSE)
...
CIdata = 
  rbind(
    spearCI[1,,drop = FALSE],
    spearCI[2,,drop = FALSE],
    spearCI[3,,drop = FALSE]
  )
ord = order(data[1,], decreasing = TRUE)
...
CIdata = 
  rbind(
    tssObsCI[1,,drop = FALSE],
    tssObsCI[2,,drop = FALSE],
    tssObsCI[3,,drop = FALSE]
  )
...
CIdata = 
  rbind(
    aucObsCI[1,,drop = FALSE],
    aucObsCI[2,,drop = FALSE],
    aucObsCI[3,,drop = FALSE]
  )
ord = order(data[1,], decreasing = TRUE)
...
data = 
  rbind(
    spearObs[1,,drop = FALSE],
    spearObs[2,,drop = FALSE],
    spearObs[3,,drop = FALSE]
  )
CIdata = 
  rbind(
    spearObsCI[1,,drop = FALSE],
    spearObsCI[2,,drop = FALSE],
    spearObsCI[3,,drop = FALSE]
  )
ord = order(data[1,], decreasing = TRUE)
...
pdf(file = "../Figures/FigS3.pdf", width = 2.25, height = 1.8)
par(mfrow = c(1,2),  mar = c(1.0,1.4,0.4, 0.3), oma = c(0,0.0,0.4,0), mgp = c(3,0.27,0))
