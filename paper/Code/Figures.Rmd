...
pdf("test_fiure2.pdf",width = 5.5, height = 2.3)
par(mfrow = c(1,3), mar = c(2,3,1,0.3), mgp = c(1.8,0.7,0), oma = c(2,0,0,0))
cex = 0.65
cexP = 0.7
...
matplot(y = t(data),
        ...
        axes = TRUE,
        xlab = "",
        ylab = "TSS", las =2,  xaxt="n", cex.axis = cex, cex.lab = cex)
...
axis(1, labels = FALSE, at = x,  cex.axis = cex, las = 2)
text(x-0.2, par("usr")[3] - 0.1, labels = dictonary[names(data)], srt = 45, pos = 1, xpd = TRUE, cex = cex)
text(x = 0, pos = 2, y = 1+0.16, xpd = NA, labels = "a)", cex = cex)
...
matplot(y = t(data),
        ...
        axes = TRUE,
        xlab = "",
        ylab = "AUC", las =2,  xaxt="n",  cex.axis = cex, cex.lab = cex)
lines(smooth.spline(y = data[1,],x = x, spar = 0.3 ), col = col[1], lwd = lwd)
lines(smooth.spline(y = data[2,],x = x+offset[2], spar = 0.3 ), col = col[2], lwd = lwd)
lines(smooth.spline(y = data[3,],x = x+offset[3], spar = 0.3 ), col = col[3], lwd = lwd)
for(i in 1:3){
...
axis(1, labels = FALSE, at = x,  cex.axis = cex, las = 2)
text(x-0.2, par("usr")[3] - 0.05, labels = dictonary[names(data)], srt = 45, pos = 1, xpd = TRUE, cex = cex)
text(x = 0, pos = 2, y = 1+0.08, xpd = NA, labels = "b)", cex = cex)


 points(x = seq(-11,10, length.out = 3), y = rep(-0.3,3), pch = pch, col = col_full, xpd = NA)
 text(x = seq(-11,10, length.out = 3), y = rep(-0.3,3), pos = 4, 
...
matplot(y = t(data),
        ...
        axes = TRUE,
        xlab = "",
        ylab = "Spearman", las =2,  xaxt="n",  cex.axis = cex, cex.lab = cex)
lines(smooth.spline(y = data[1,],x = x, spar = 0.3 ), col = col[1], lwd = lwd)
lines(smooth.spline(y = data[2,],x = x+offset[2], spar = 0.3 ), col = col[2], lwd = lwd)
lines(smooth.spline(y = data[3,],x = x+offset[3], spar = 0.3 ), col = col[3], lwd = lwd)
...
axis(1, labels = FALSE, at = x,  cex.axis = cex, las = 2)
text(x-0.17, par("usr")[3] - 0.1, labels = dictonary[names(data)], srt = 45, pos = 1, xpd = TRUE, cex = cex)
text(x = 0, pos = 2, y = 1+0.16, xpd = NA, labels = "c)", cex = cex)
...
plotInf = function(res,ci, xlab = "", ylab = "", col = rep("black", 7)){
  nC = ncol(res)
  xX = 1/(nC+1)
  xS = xX/(nC-1)
  nR = nrow(res)
  ...
  for(i in 1:nC){
    tmp = res[,i]
    for(j in 1:nR){
      points(x = xC[i]+(j-1)*xSS, y = tmp[j], col = col[i], pch = 16)
      coords[counter] = mean(c(xC[i]+(j-1)*xSS, xC[i]+j*xSS))
      counter = counter + 1
      plotCI(x = xC[i]+(j-1)*xSS, y = tmp[j], ci[j,i], col = "black")
  ...
  for(i in 1:5){
    ...
    cc= c(xC[i] - xX*0.08, (xC[i])+xX*1.08)
    
    segments(x0 = min(cc), x1 = max(cc), y0 = ce[1]-mean(cc)*ce[2], y1 = ce[1]+mean(cc)*ce[2], lty =2 , col = "red")
  }
  
  
  return(coords)
}
coords = plotInf(ResClassT[,ord], ResClassTCI[,ord])
