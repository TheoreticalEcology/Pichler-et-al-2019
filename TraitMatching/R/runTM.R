...
parallelSetup <- function(settings) {
  ...
  if(!parallel == FALSE){
    ...
    if ("dnn" %in% method) parallelMap::parallelExport("trainLearner.classif.keras_seq", "predictLearner.classif.keras_seq", level = "mlr.tuneParams")
    if ("cnn" %in% method) parallelMap::parallelExport("trainLearner.classif.keras_conv", "predictLearner.classif.keras_conv", level = "mlr.tuneParams")
    if ("wideDeep" %in% method) parallelMap::parallelExport("trainLearner.classif.wideDeep", "predictLearner.classif.wideDeep", level = "mlr.tuneParams")
    if ("preDnn" %in% method) parallelMap::parallelExport("trainLearner.classif.preKeras", "predictLearner.classif.preKeras", level = "mlr.tuneParams")
    if ("SVM" %in% method) parallelMap::parallelExport("trainLearner.classif.liquidSVM_self", "predictLearner.classif.liquidSVM_self", level = "mlr.tuneParams")

    if ("dnn" %in% method) parallelMap::parallelExport("trainLearner.regr.keras_seq", "predictLearner.regr.keras_seq", level = "mlr.tuneParams")
    if ("cnn" %in% method) parallelMap::parallelExport("trainLearner.regr.keras_conv", "predictLearner.regr.keras_conv", level = "mlr.tuneParams")
    if ("wideDeep" %in% method) parallelMap::parallelExport("trainLearner.regr.wideDeep", "predictLearner.regr.wideDeep", level = "mlr.tuneParams")
    if ("preDnn" %in% method) parallelMap::parallelExport("trainLearner.regr.preKeras", "predictLearner.regr.preKeras", level = "mlr.tuneParams")
    if("negBinDnn"%in% method )parallelMap::parallelExport("trainLearner.regr.negBinDnn", "predictLearner.regr.negBinDnn", level = "mlr.tuneParams")
    if("multiNomDnn"%in% method )parallelMap::parallelExport("trainLearner.regr.multiNomDnn", "predictLearner.regr.multiNomDnn", level = "mlr.tuneParams")
    if("poissonDnn"%in% method )parallelMap::parallelExport("trainLearner.regr.poissonDnn", "predictLearner.regr.poissonDnn", level = "mlr.tuneParams")
  }
}
